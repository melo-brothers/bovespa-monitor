...
def day_summary(user_id: int):
    ...
    for symbol in symbols:
        data = get(
            f"https://www.mercadobitcoin.net/api/{symbol}/day-summary/{today.year}/{today.month}/{today.day}/"
        ).json()
        result.append({"symbol": symbol, "lowest": data["lowest"], "highest": data["highest"]})
